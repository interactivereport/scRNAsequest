...
[config.yml](https://github.com/interactivereport/scRNAsequest/blob/main/src/template.yml)
...
ref_name:                         #optional. You can choose one from scAnalyzer call without argument
...
The sample meta file, usually named as **sampleMeta.csv**, is a csv file storing data information. The minimal columns for this file are **Sample_Name** (This name can be changed, but it must be consistent with the sample_name in the config.yml file) and **h5path**. If you have the celltype annotation information, you can provide is by adding a column called **metapath**. For other information, you can add as many columns in this csv file and the column names can be defined by yourself.

### h5 input

Here is an example with minimal information for the public dataset [E-MTAB-11115](https://www.ebi.ac.uk/arrayexpress/experiments/E-MTAB-11115/) (10X h5 format):
...
Another example if you would like to use the celltype annotation defined by their analysis, which can be included in the **metapath** column. Moreover, the user has the freedom to provide further information such as sex and age:
...
E-MTAB-11115/
    ├── data
        ├── 5705STDY8058280_annotation.csv           (optional)
        ├── 5705STDY8058280_raw_feature_bc_matrix.h5
        ├── 5705STDY8058281_annotation.csv           (optional)
